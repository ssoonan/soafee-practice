...
def _lane_lines(image, lines):
    ...
    """
    left_lane, right_lane = _average_slope_intercept(lines)
    y1 = image.shape[0]
    y2 = y1 * 0.6

    # Get pixel points for left and right lanes
    left_line = _pixel_points(y1, y2, left_lane)
    right_line = _pixel_points(y1, y2, right_lane)
...
def enhanced_lane_detection(image: cv2.typing.MatLike):
    ...
    left_line, right_line = _lane_lines(image, lines)
